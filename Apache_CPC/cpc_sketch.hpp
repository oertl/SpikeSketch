...
public:
   ...
   */
  explicit cpc_sketch_alloc(uint32_t k = cpc_constants::DEFAULT_K, uint64_t seed = DEFAULT_SEED, const A& allocator = A());
...
private:
  ...
  static inline flavor determine_flavor(uint32_t k, uint64_t c);
  ...
  static void check_k(uint32_t k);
