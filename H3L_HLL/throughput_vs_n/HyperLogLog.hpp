...
namespace hyperlogloglog {
  ...
  public:
    ...
    inline void add(const Object& o) {
...
//      uint64_t hashValues[2];
      uint32_t x=MurmurHash2(&o,8,1316561);
        uint64_t y=MurmurHash64A(&o,8,51611);
//      addHash(hashValues[0], hashValues[1]);
        addJr(x%m, rho(y));
    }
...
//     */
//    inline void addHash(Word x, Word y) {
////      static_assert(::std::is_same<decltype(f(x,m)),Word>::value,
////                    "Hash function type does not match the Word type of the class");
//      addJr(x%m, rho(y));
//    }
